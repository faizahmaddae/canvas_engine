...
///
/// Every interaction goes through [SetImageShadowCommand] which
...
/// reset opacity, etc.
class ImageShadowBody extends ConsumerStatefulWidget {
  const ImageShadowBody({super.key, required this.layer});

  final ImageLayer layer;

  @override
  ConsumerState<ImageShadowBody> createState() => _ImageShadowBodyState();
}

class _ImageShadowBodyState extends ConsumerState<ImageShadowBody> {
...
_ShadowPreset? _matchPreset(ImageLayer layer) {
...
class _DirectionCell extends StatelessWidget {
      ...
      child: InkWell(
        ...
        child: Center(
          child: isCenter
              ...
              : Transform.rotate(
                  // Arrow points outward from the centre.
  ...
  /// Angle (radians) so `arrow_upward` points outward from centre
  /// of the pad to this cell. `arrow_upward` natively points up
  /// (toward `(0, -1)`), so we rotate it to align with `(dx, dy)`.
  double _arrowAngle(int dx, int dy) {
    // `arrow_upward` is drawn pointing toward (0, -1). Rotating by
    // atan2(dy, dx) + pi/2 sends the tip toward (dx, dy).
