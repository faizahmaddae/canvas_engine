...
void main() {
  group('continuous pinch \u2014 single layer', () {
    test('long continuous pinch-out asymptotes at the per-frame '
      ...
      ctl.startGesture(layer: layer, focalPoint: const Offset(400, 400));
      ...
      expect(
        ...
        reason:
            'continuous pinch must saturate at exactly '
    ...
    test('long continuous pinch-in asymptotes at the per-frame '
      ...
      ctl.startGesture(layer: layer, focalPoint: const Offset(600, 600));
      ...
      expect(
        ...
        reason:
            'continuous pinch-in must saturate at exactly '
    ...
    test('pinch-out then pinch-in within a single gesture follows the '
      ...
      final big = c.read(interactionControllerProvider).liveTransform!;
      ...
      expect(
        ...
        closeTo(60 * EngineConstants.maxGestureScale, 0.01),
        reason:
            'pinch-out past the cap saturates at '
      ...
      final small = c.read(interactionControllerProvider).liveTransform!;
      ...
      expect(
        ...
        closeTo(60 * 0.5, 0.01),
        reason:
            'pinch-in within the same gesture must follow '
            'the cumulative scale back down once it re-enters '
            'the clamp window',
      );
    });

    test('stray pointerCount=1 frame mid-pinch rebases the session and '
      ...
      final beforeGlitch = c.read(interactionControllerProvider).liveTransform!;
      ...
      final afterGlitch = c.read(interactionControllerProvider).liveTransform!;
      expect(
        afterGlitch.size.width,
        closeTo(beforeGlitch.size.width, 0.01),
        reason:
            'rebase-on-transition snapshots the size; the '
      ...
      final afterResume = c.read(interactionControllerProvider).liveTransform!;
      expect(
        afterResume.size.width,
        closeTo(afterGlitch.size.width * EngineConstants.maxGestureScale, 0.01),
        reason:
            'after the glitch + rebase, the per-frame clamp '
            'caps growth at `currentSize * maxGestureScale`; the '
            'session is alive throughout (no forced release)',
      );
    });

    test('multi-touch is direct-manipulation: liveTransform exactly '
      ...
      expect(
        ...
        closeTo(60 * 1.5, 0.01),
        reason:
            'multi-touch must not be smoothed: live size '
    ...
    test('transient pointer drop+re-add mid-pinch does NOT lose scale '
      ...
      final afterReadd = c.read(interactionControllerProvider).liveTransform!;
      expect(
        afterReadd.size.width,
        closeTo(beforeGlitch.size.width, 0.5),
        reason:
            'pinch must NOT lose progress on a transient '
      ...
      expect(
        ...
        reason:
            'after the glitch, further finger spread must '
            'multiply against the current live size, not the '
            'original pre-pinch size',
      );
    });

    test('fast pinch with relScale > maxGestureScale is clamped per-frame '
      ...
      final afterFast = c.read(interactionControllerProvider).liveTransform!;
      expect(
        afterFast.size.width,
        closeTo(60 * EngineConstants.maxGestureScale, 0.01),
        reason:
            'a single fast pinch frame must be clamped to '
      ...
      final afterMore = c.read(interactionControllerProvider).liveTransform!;
      expect(
        afterMore.size.width,
        closeTo(60 * EngineConstants.maxGestureScale, 0.01),
        reason:
            'subsequent fast frames stay clamped against the '
  ...
  group('continuous pinch — multi-select / group', () {
    test('long continuous group pinch-out keeps scaling the whole '
      ...
      ctl.startGroupGesture(layers: [a, b], focalPoint: const Offset(150, 50));
      ...
      expect(
        ...
        greaterThan(100 * 10),
        reason:
            'group pinch must keep growing each member past '
            'the per-frame clamp',
      );
    ...
    test('long continuous group pinch-in keeps shrinking each member '
      ...
      ctl.startGroupGesture(layers: [a, b], focalPoint: const Offset(450, 200));
      ...
      expect(
        ...
        lessThan(60),
        reason:
            'group pinch-in must keep shrinking past the '
    ...
    test('stray pointerCount=1 frame mid-group-pinch does NOT collapse '
      ...
      expect(
        ...
        beforeA,
        reason:
            'stray 1-pointer frame must be ignored by the '
            'group session',
      );
      ...
      expect(
        afterA.size.width,
        greaterThan(beforeA.size.width * 2),
        reason: 'group pinch must resume growing after the glitch',
      );
    });

    test('genuine one-finger drag on a freshly started session still '
      ...
      ctl.startGesture(layer: layer, focalPoint: const Offset(130, 130));
