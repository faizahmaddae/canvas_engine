...
void main() {
  testWidgets(
    'tapping a higher overlapping layer switches selection directly '
    '(no manual deselect required)',
    (tester) async {
      ...
      expect(
        container.read(selectionControllerProvider).selectedId,
        'top',
        reason: 'A single tap on a visually-higher overlapping layer '
  ...
  testWidgets(
    'tapping the selected layer in a region where no higher layer '
    'overlaps keeps selection (no spurious switch / clear)',
    (tester) async {
      tester.view.physicalSize = const Size(800, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
  ...
  testWidgets(
    'a higher LOCKED overlapping layer does NOT block selecting / '
    'manipulating the lower selected layer',
    (tester) async {
      tester.view.physicalSize = const Size(800, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      expect(
        container.read(selectionControllerProvider).selectedId,
        'bottom',
        reason: 'A locked higher layer must NOT block selection of the '
            'lower layer.',
      );
    },
  );
