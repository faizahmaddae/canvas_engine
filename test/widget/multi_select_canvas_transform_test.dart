...
void main() {
  testWidgets('one-finger drag on empty canvas moves the whole group '
    ...
    expect(
      ...
      isNotNull,
      reason:
          'One-finger off-group drag must promote the deferred '
  ...
  testWidgets('two-finger pinch starting entirely outside the group transforms '
      'the group (viewport stays put)', (tester) async {
    final container = _setup(tester);
    await _pumpEditorWithGroup(tester, container);
    ...
    expect(
      ...
      isNull,
      reason:
          'Deferred-claim: first off-group pointer must NOT '
          'start a session yet.',
    );
  ...
  testWidgets('pause-then-drag on empty canvas with a group selected still '
    ...
    expect(
      ...
      isNotNull,
      reason:
          'Pause-then-drag must still promote to a group '
