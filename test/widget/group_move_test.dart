...
void main() {
  ...
  group('group move', () {
    test('startGroupMove seeds initials for every eligible layer', () {
      ...
      c
          ...
          .startGroupMove(layers: [a, b, cc], pointer: const Offset(140, 140));
    ...
    test('updateGroup translates every layer by the same delta', () {
      ...
      c
          .read(interactionControllerProvider.notifier)
          .startGroupMove(layers: [a, b], pointer: const Offset(140, 140));
      c
          .read(interactionControllerProvider.notifier)
          .updateGroup(
    ...
    test('end commits a CompositeCommand; single undo restores ALL layers', () {
      ...
      c
          .read(interactionControllerProvider.notifier)
          .startGroupMove(layers: [a, b, cc], pointer: const Offset(0, 0));
      c
          .read(interactionControllerProvider.notifier)
          .updateGroup(const Offset(100, 100));
      c.read(interactionControllerProvider.notifier).end();

      final docAfter = c.read(documentControllerProvider);
      expect(
        docAfter.layerById('a')!.transform.position,
        a.transform.position + const Offset(100, 100),
      );
      expect(
        docAfter.layerById('b')!.transform.position,
        b.transform.position + const Offset(100, 100),
      );
      expect(
        docAfter.layerById('cc')!.transform.position,
      ...
      expect(
        docUndone.layerById('a')!.transform.position,
        a.transform.position,
      );
      expect(
        docUndone.layerById('b')!.transform.position,
        b.transform.position,
      );
      expect(
        docUndone.layerById('cc')!.transform.position,
    ...
    test('cancel mid-drag commits nothing for ANY layer', () {
      ...
      c
          .read(interactionControllerProvider.notifier)
          .startGroupMove(layers: [a, b], pointer: const Offset(0, 0));
      c
          .read(interactionControllerProvider.notifier)
          .updateGroup(const Offset(100, 100));
  ...
  group('group resize', () {
    test('uniform scale 2x from bottom-right anchor doubles bounds', () {
      ...
      c
          .read(interactionControllerProvider.notifier)
          .updateGroup(const Offset(-300, -100));
      c.read(interactionControllerProvider.notifier).end();
  ...
  group('group rotate', () {
    test('rotates every layer 90° around the group centre', () {
      ...
      c
          .read(interactionControllerProvider.notifier)
          .startGroupRotate(layers: [a, b], pointer: const Offset(250, 50));
      c
          .read(interactionControllerProvider.notifier)
          .updateGroup(const Offset(150, 150));
      c.read(interactionControllerProvider.notifier).end();
  ...
  group('group gesture (pinch)', () {
    test('pure scale=2 around bounds centre doubles each layer', () {
      ...
      c
          .read(interactionControllerProvider.notifier)
          .startGroupGesture(layers: [a, b], focalPoint: const Offset(150, 50));
      c
          .read(interactionControllerProvider.notifier)
          .updateGroupGesture(
  ...
  group('hardening', () {
    test('group resize cannot flip layers (factor clamped positive)', () {
      ...
      c
          .read(interactionControllerProvider.notifier)
          .updateGroup(const Offset(900, 300));
      c.read(interactionControllerProvider.notifier).end();
    ...
    test('group resize never shrinks any layer below minLayerSize', () {
      ...
      c
          .read(interactionControllerProvider.notifier)
          .updateGroup(bounds.bottomRight - const Offset(1, 1));
      c.read(interactionControllerProvider.notifier).end();
    ...
    test('group rotate handles ±π branch cut without 2π jump', () {
      ...
      c
          .read(interactionControllerProvider.notifier)
          .startGroupRotate(layers: [a, b], pointer: const Offset(50, 49.99));
      // Pointer just above -x axis: angle ≈ +π-ε. Naive delta ≈ 2π.
      c
          .read(interactionControllerProvider.notifier)
          .updateGroup(const Offset(50, 50.01));
  ...
  group('group drag snap (gesture-session path \u2014 the real UI route)', () {
    test('single-finger drag on a group session snaps the AABB to a peer '
      ...
      expect(
        ...
        isNotEmpty,
        reason:
            'group single-finger drag must publish alignment '
    ...
    test('two-finger pinch on a group session does NOT publish snap '
  ...
  group('group selection frame (rotated chrome)', () {
    test('rotate handle drag publishes a quad whose corners are '
      ...
      c
          .read(interactionControllerProvider.notifier)
          .updateGroup(const Offset(150, 200));
    ...
    test('translate-only gesture publishes an axis-aligned quad '
    ...
    test('pinch with rotation publishes a quad rotated by the same '
  ...
  group('group selection frame clamp tracking', () {
    ...
    test('clamped single-handle resize: frame uses applied 0.8, not '
        'requested 0.4', () {
      final c = makeContainer();
      seedSmallAndBig(c);
      final doc = c.read(documentControllerProvider);
    ...
    test('continued drag beyond the clamp keeps frame synced to layers on '
        'every update', () {
      final c = makeContainer();
      seedSmallAndBig(c);
      final doc = c.read(documentControllerProvider);
      ...
      for (final p in const <Offset>[
        ...
        final s = c.read(interactionControllerProvider);
        expect(
          s.groupLive['small']!.size.width,
          closeTo(24, 1e-6),
          reason: 'small stays floored at 24 for pointer $p',
        );
        expect(
          s.groupLiveBounds,
          const Rect.fromLTRB(0, 0, 320, 240),
    ...
    test('clamped pinch with rotation: oriented quad uses applied 0.8 and '
    ...
    test('pointer-up commits geometry that matches the final live frame '
        '(clamped resize)', () {
      final c = makeContainer();
      seedSmallAndBig(c);
  ...
  group('commit hardening', () {
    test('pinch session with 1\u21922\u21921\u21922 pointer oscillation '
      ...
      final docAfter = c.read(documentControllerProvider);
      expect(
        docAfter.layerById('a')!.transform.size.width,
        greaterThan(100),
        reason:
            'Oscillating pointer counts must not prevent the '
    ...
    test('group rotate commits the rotation on release', () {
      ...
      c
          .read(interactionControllerProvider.notifier)
          .updateGroup(const Offset(140, 180));
      c.read(interactionControllerProvider.notifier).end();
    ...
    test('gesture that returns to origin pushes NO history entry', () {
      ...
      final doc = c.read(documentControllerProvider);
      final undoBefore = c.read(documentControllerProvider.notifier).canUndo;
      ...
      expect(
        ...
        undoBefore,
        reason:
            'A gesture that did not actually change any layer '
    ...
    test('undo mid-drag cancels the active group session cleanly', () {
          ...
          .execute(
            SetLayerTransformCommand(
              ...
              transform: doc
                  .layerById('a')!
                  .transform
                  .copyWith(position: const Offset(10, 0)),
            ),
          );
      ...
      final ctrl = c.read(interactionControllerProvider.notifier);
      ctrl.startGroupMove(layers: [aSeeded, bSeeded], pointer: Offset.zero);
      ...
      expect(
        c.read(interactionControllerProvider).isActive,
        isFalse,
        reason:
            'An external transform change must cancel the '
