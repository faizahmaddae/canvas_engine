...
void main() {
  group('sticker layer model independence', () {
    test('TextLayer.copyWith preserves transform / visibility / locked / '
        'opacity / kind', () {
    ...
    test('resizing sticker B does not move or resize sticker A', () {
      final c = _container();
      final a = _sticker(
        id: 'A',
        emoji: '\u{1F600}',
  ...
  group('sticker layer rendering keys', () {
    ...
    testWidgets('sticker render path does NOT paint via Text widgets at all '
        '(Impeller color-glyph atlas-corruption guard)', (tester) async {
        ...
        MaterialApp(
          home: Directionality(
            ...
            child: SizedBox(
              ...
              height: 200,
              child: Stack(
                children: [LayerRenderer(layer: s, transform: s.transform)],
              ),
            ),
          ),
        ),
      ...
      expect(
        ...
        findsNothing,
        reason:
            'sticker glyphs must be pre-rasterised to a ui.Image '
      ...
      expect(
        ...
        findsWidgets,
        reason:
            'sticker body must be rendered through RawImage so it '
