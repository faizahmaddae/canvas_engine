...
void main() {
  group('Layers panel delete', () {
    testWidgets('delete is DISABLED for protected base photo',
        (tester) async {
    ...
    testWidgets('overlay row deletable; base-photo row not',
        (tester) async {
      final c = _photoProject();
      addTearDown(c.dispose);
      c
  ...
  group('LayerActions.delete (programmatic)', () {
    ...
    testWidgets('overlay image in photo project deletes WITHOUT confirm',
      ...
      await _withRef(tester, c, (ctx, ref) async {
        final layer =
            c.read(documentControllerProvider).layerById('overlay')!;
